...
import { markAdminHandoffFailed } from "../../../../../lib/admin-dashboard-actions";
import { ADMIN_OPERATION_BODY_BYTES, isAdminUuid, validateHandoffFailInput } from "../../../../../lib/admin-dashboard-validation";
...
export async function POST(request: Request, context: { params: Promise<{ id: string }> }) {
  ...
  const validation = validateHandoffFailInput(body.data);
  if (validation.success === false) return adminJsonResponse({ success: false, error: validation.error }, 400);
  try {
    await markAdminHandoffFailed(createPrivilegedSupabaseClient(), { handoffId: id, ...validation.data });
    return adminJsonResponse({ success: true });
  } catch (error) {
    return error instanceof Error && error.message === "Trusted database operation failed."
      ? adminOperationFailed("admin_handoff_fail_failed")
      : adminServiceUnavailable("admin_handoff_fail_unavailable");
  }
}
