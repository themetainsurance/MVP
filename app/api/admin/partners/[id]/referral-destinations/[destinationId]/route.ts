...
export async function PATCH(request: Request, context: { params: Promise<{ id: string; destinationId: string }> }) {
  ...
  try {
    ...
    if (existing.error) return adminOperationFailed("partner_referral_destination_update_failed");
    if (!existing.data) return adminJsonResponse({ success: false, error: "Record not found." }, 404);
    const { data, error } = await client.rpc("update_partner_referral_destination", {
    ...
    if (error || data !== true) return adminOperationFailed("partner_referral_destination_update_failed");
    return adminJsonResponse({ success: true });
  } catch {
    return adminServiceUnavailable("partner_referral_destination_update_unavailable");
  }
}
