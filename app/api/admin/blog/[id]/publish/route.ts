...
export async function POST(
  ...
  try {
    ...
    const validation = validatePublishableBlogPost(current);
    ...
    const { data, error } = await client
      ...
      .select("id")
      .maybeSingle();
    if (error) return adminOperationFailed("admin_blog_publish_failed");
    if (!data) {
      return adminJsonResponse(
        { success: false, error: "Blog post not found." },
        404
    ...
    return error instanceof Error && error.message === "blog_cms_unavailable"
      ? adminServiceUnavailable("admin_blog_publish_schema_unavailable")
      : adminServiceUnavailable("admin_blog_publish_unavailable");
  }
}
